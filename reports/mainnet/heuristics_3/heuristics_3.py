...
from reports.mainnet.heuristics import Heuristics
...
class Heuristics3(Heuristics):
    ...
    def _get_results(self) -> list:
            ...
            if d["functionName"] == fn.S_ETH.value:
                ...
                for tx in d[d["functionName"]][stealth]:
                    ...
                    if tx["to"] not in result_dict: 
                        result_dict[tx["to"]] = {}
                        if "receiver_ens" in tx: result_dict[tx["to"]]["ens"] = tx["receiver_ens"]
                ...
                if receiver not in result_dict: 
                    ...
                    if "receiver_ens" in func: result_dict[receiver]["ens"] = func["receiver_ens"]
        ...
        with open("reports/mainnet/results/collector_pattern.json", "w") as file:
